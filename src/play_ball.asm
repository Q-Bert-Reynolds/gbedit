...
DOUBLE_CALC_A: macro
  ...
  srl a;b >> 3
  and a, 6;(b>>3) & 6
  or b;(b&128)|((b>>1)&96)|((b>>2)&24)|((b>>3)&6)
  ld b, a
  ld a, [_b]
  swap a;similar to b >> 4, but leaves upper nibble
  and a, 1;(b>>4) & 1
  or b;(b&128)|((b>>1)&96)|((b>>2)&24)|((b>>3)&6)|((b>>4)&1)
ENDM

DOUBLE_CALC_B: macro
  ld a, [_b]
  swap a;equivalent to b << 4
  and a, 128;(b<<4)&128
  ld b, a
  ld a, [_b]
  sla a
  ...
  sla a ;b << 2
  and 24;(b<<2) & 24
  or b ;((b<<4)&128)|((b<<3)&96)|((b<<2)&24)
  ld b, a
  ld a, [_b]
  sla a;b << 1
  and 6;(b<<1) & 6
  or b;((b<<4)&128)|((b<<3)&96)|((b<<2)&24)|((b<<1)&6)
  ld b, a
  ld a, [_b]
  and 1;b & 1
  or b;((b<<4)&128)|((b<<3)&96)|((b<<2)&24)|((b<<1)&6)|(b&1)
ENDM
...
SetBkgDataDoubled: ;de = vram location, bc = nb_tiles, hl = data
  ...
  ld a, c;assumes less than 256 tiles
  dec a
  ld [_i], a
...
.rowLoop ;for (j = 0; j < 8; j+=2) {
      ...
      ld a, [hli]; +1
      ld [_b], a;b = data[i*16+j];
      push hl;data[i*16+j+1];

      ld de, 64
      ld a, [_i]
      call math_Multiply;i*64
      ld b, 0
      ld a, [_j]
      add a, a;j*2
      ld c, a
      add hl, bc;i*64+j*2
      ld de, tile_buffer
      add hl, de;tiles[i*64+j*2]
      push hl;tiles[i*64+j*2]
      DOUBLE_CALC_A
      ld [hli], a;tiles[i*64+j*2] = (b&128)|((b>>1)&96)|((b>>2)&24)|((b>>3)&6)|((b>>4)&1);
      inc hl ;+2
      ld [hl], a;tiles[i*64+j*2+2] = tiles[i*64+j*2];
      
      ld bc, 14
      add hl, bc
      DOUBLE_CALC_B
      ld [hli], a;tiles[i*64+j*2+16] = ((b<<4)&128)|((b<<3)&96)|((b<<2)&24)|((b<<1)&6)|(b&1);
      inc hl ;+18
      ld [hl], a;tiles[i*64+j*2+18] = tiles[i*64+j*2+16];

      pop hl;tiles[i*64+j*2]
      pop de;data[i*16+j+1];
      ld a, [de]
      ld [_b], a;b = data[i*16+j+1];
      inc de;+2
      inc de;+3
      inc de;+4
      inc de;+5
      ...
      inc de;+7
      inc de;+8
      push de;data[i*16+j+8]

      DOUBLE_CALC_A
      push hl;tiles[i*64+j*2]
      inc hl
      ld [hli], a;tiles[i*64+j*2+1]  = (b&128)|((b>>1)&96)|((b>>2)&24)|((b>>3)&6)|((b>>4)&1);
      inc hl
      ld [hl], a;tiles[i*64+j*2+3]  = tiles[i*64+j*2+1];
      ld bc, 14
      add hl, bc
      DOUBLE_CALC_B
      ld [hli], a;tiles[i*64+j*2+17] = ((b<<4)&128)|((b<<3)&96)|((b<<2)&24)|((b<<1)&6)|(b&1);
      inc hl
      ld [hl], a;tiles[i*64+j*2+19] = tiles[i*64+j*2+17];
      
      pop hl;tiles[i*64+j*2]
      pop de;data[i*16+j+8]
      ld a, [de]
      ld [_b], a;b = data[i*16+j+8];
      inc de;;+9
      push de;data[i*16+j+9];

      DOUBLE_CALC_A
      push hl;tiles[i*64+j*2]
      ld bc, 32
      add hl, bc
      ld [hli], a;tiles[i*64+j*2+32] = (b&128)|((b>>1)&96)|((b>>2)&24)|((b>>3)&6)|((b>>4)&1);
      inc a
      ld [hl], a;tiles[i*64+j*2+34] = tiles[i*64+j*2+32];
      ld bc, 14
      add hl, bc
      DOUBLE_CALC_B
      ld [hli], a;tiles[i*64+j*2+48] = ((b<<4)&128)|((b<<3)&96)|((b<<2)&24)|((b<<1)&6)|(b&1);
      inc hl
      ld [hl], a;tiles[i*64+j*2+50] = tiles[i*64+j*2+48];
      
      pop hl;tiles[i*64+j*2]
      pop de;data[i*16+j+9];
      ld a, [de]
      ld [_b], a;b = data[i*16+j+9];

      DOUBLE_CALC_A
      ld bc, 33
      add hl, bc
      ld [hli], a;tiles[i*64+j*2+33] = (b&128)|((b>>1)&96)|((b>>2)&24)|((b>>3)&6)|((b>>4)&1);
      inc hl
      ld [hl], a;tiles[i*64+j*2+35] = tiles[i*64+j*2+33];
      ld bc, 14
      add hl, bc
      DOUBLE_CALC_B
      ld [hli], a;tiles[i*64+j*2+49] = ((b<<4)&128)|((b<<3)&96)|((b<<2)&24)|((b<<1)&6)|(b&1);
      inc hl
      ld [hl], a;tiles[i*64+j*2+51] = tiles[i*64+j*2+49];
  ...
  ld c, l ;bc = tiles*64
  ld hl, tile_buffer
  call mem_CopyVRAM; set_bkg_data(vram, nb_tiles*64, tiles);
  ret
